from tkinter import *
from tkinter import filedialog
from customtkinter import *
from PIL import Image
from random import randrange

def ouvrir_fichier():
    """donne le chemin d'acces de l'image qui a pour extension .png ou .jpg
    ENTREE: rien
    SORTIE: chemin acces d'une seule image qui a pour extension .png ou .jpg"""
    global nomfichier
    nomfichier=filedialog.askopenfilename(parent=fenetre,title="Choisissez un fichier.",filetypes=[(".png",".jpg")],multiple=NO)

def imagefinale():
    """retourne image finale
    ENTREE: rien
    SORTIE: une image composee des images choisies par l'utilisateur"""
    global new_im
    img=Image.open(nomfichier)
    largeur,hauteur=img.size
    new_im=Image.new('RGB',(largeur*nb_colonnes,hauteur*nb_lignes))
    bruit()
    niveaudegris()
    noiretblanc()
    negatif()
    changercouleur1()
    changercouleur2()

    #img00
    if img00.get()=="Image de base":
        if img00r.get()==1:
            retourner(img)
            new_im.paste(imgretourner,(0*largeur,0*hauteur))
        elif img00m.get()==1:
            miroir(img)
            new_im.paste(imgmiroir,(0*largeur,0*hauteur))
        else:
            new_im.paste(img,(0*largeur,0*hauteur))
    if img00.get()=="Bruit":
        if img00r.get()==1:
            retourner(imgbruit)
            new_im.paste(imgretourner,(0*largeur,0*hauteur))
        elif img00m.get()==1:
            miroir(imgbruit)
            new_im.paste(imgmiroir,(0*largeur,0*hauteur))
        else:
            new_im.paste(imgbruit,(0*largeur,0*hauteur))
    if img00.get()=="Niveau de gris":
        if img00r.get()==1:
            retourner(imggris)
            new_im.paste(imgretourner,(0*largeur,0*hauteur))
        elif img00m.get()==1:
            miroir(imggris)
            new_im.paste(imgmiroir,(0*largeur,0*hauteur))
        else:
            new_im.paste(imggris,(0*largeur,0*hauteur))
    if img00.get()=="Noir et blanc":
        if img00r.get()==1:
            retourner(imgnoiretblanc)
            new_im.paste(imgretourner,(0*largeur,0*hauteur))
        elif img00m.get()==1:
            miroir(imgnoiretblanc)
            new_im.paste(imgmiroir,(0*largeur,0*hauteur))
        else:
            new_im.paste(imgnoiretblanc,(0*largeur,0*hauteur))
    if img00.get()=="Negatif":
        if img00r.get()==1:
            retourner(imgnegatif)
            new_im.paste(imgretourner,(0*largeur,0*hauteur))
        elif img00m.get()==1:
            miroir(imgnegatif)
            new_im.paste(imgmiroir,(0*largeur,0*hauteur))
        else:
            new_im.paste(imgnegatif,(0*largeur,0*hauteur))
    if img00.get()=="Mofication couleurs 1":
        if img00r.get()==1:
            retourner(imgchangercouleur1)
            new_im.paste(imgretourner,(0*largeur,0*hauteur))
        elif img00m.get()==1:
            miroir(imgchangercouleur1)
            new_im.paste(imgmiroir,(0*largeur,0*hauteur))
        else:
            new_im.paste(imgchangercouleur1,(0*largeur,0*hauteur))
    if img00.get()=="Mofication couleurs 2":
        if img00r.get()==1:
            retourner(imgchangercouleur2)
            new_im.paste(imgretourner,(0*largeur,0*hauteur))
        elif img00m.get()==1:
            miroir(imgchangercouleur2)
            new_im.paste(imgmiroir,(0*largeur,0*hauteur))
        else:
            new_im.paste(imgchangercouleur2,(0*largeur,0*hauteur))

    #img01
    if img01.get()=="Image de base":
        if img01r.get()==1:
            retourner(img)
            new_im.paste(imgretourner,(0*largeur,1*hauteur))
        elif img01m.get()==1:
            miroir(img)
            new_im.paste(imgmiroir,(0*largeur,1*hauteur))
        else:
            new_im.paste(img,(0*largeur,1*hauteur))
    if img01.get()=="Bruit":
        if img01r.get()==1:
            retourner(imgbruit)
            new_im.paste(imgretourner,(0*largeur,1*hauteur))
        elif img01m.get()==1:
            miroir(imgbruit)
            new_im.paste(imgmiroir,(0*largeur,1*hauteur))
        else:
            new_im.paste(imgbruit,(0*largeur,1*hauteur))
    if img01.get()=="Niveau de gris":
        if img01r.get()==1:
            retourner(imggris)
            new_im.paste(imgretourner,(0*largeur,1*hauteur))
        elif img01m.get()==1:
            miroir(imggris)
            new_im.paste(imgmiroir,(0*largeur,1*hauteur))
        else:
            new_im.paste(imggris,(0*largeur,1*hauteur))
    if img01.get()=="Noir et blanc":
        if img01r.get()==1:
            retourner(imgnoiretblanc)
            new_im.paste(imgretourner,(0*largeur,1*hauteur))
        elif img01m.get()==1:
            miroir(imgnoiretblanc)
            new_im.paste(imgmiroir,(0*largeur,1*hauteur))
        else:
            new_im.paste(imgnoiretblanc,(0*largeur,1*hauteur))
    if img01.get()=="Negatif":
        if img01r.get()==1:
            retourner(imgnegatif)
            new_im.paste(imgretourner,(0*largeur,1*hauteur))
        elif img01m.get()==1:
            miroir(imgnegatif)
            new_im.paste(imgmiroir,(0*largeur,1*hauteur))
        else:
            new_im.paste(imgnegatif,(0*largeur,1*hauteur))
    if img01.get()=="Mofication couleurs 1":
        if img01r.get()==1:
            retourner(imgchangercouleur1)
            new_im.paste(imgretourner,(0*largeur,1*hauteur))
        elif img01m.get()==1:
            miroir(imgchangercouleur1)
            new_im.paste(imgmiroir,(0*largeur,1*hauteur))
        else:
            new_im.paste(imgchangercouleur1,(0*largeur,1*hauteur))
    if img01.get()=="Mofication couleurs 2":
        if img01r.get()==1:
            retourner(imgchangercouleur2)
            new_im.paste(imgretourner,(0*largeur,1*hauteur))
        elif img01m.get()==1:
            miroir(imgchangercouleur2)
            new_im.paste(imgmiroir,(0*largeur,1*hauteur))
        else:
            new_im.paste(imgchangercouleur2,(0*largeur,1*hauteur))

    #img02
    if img02.get()=="Image de base":
        if img02r.get()==1:
            retourner(img)
            new_im.paste(imgretourner,(0*largeur,2*hauteur))
        elif img02m.get()==1:
            miroir(img)
            new_im.paste(imgmiroir,(0*largeur,2*hauteur))
        else:
            new_im.paste(img,(0*largeur,2*hauteur))
    if img02.get()=="Bruit":
        if img02r.get()==1:
            retourner(imgbruit)
            new_im.paste(imgretourner,(0*largeur,2*hauteur))
        elif img02m.get()==1:
            miroir(imgbruit)
            new_im.paste(imgmiroir,(0*largeur,2*hauteur))
        else:
            new_im.paste(imgbruit,(0*largeur,2*hauteur))
    if img02.get()=="Niveau de gris":
        if img02r.get()==1:
            retourner(imggris)
            new_im.paste(imgretourner,(0*largeur,2*hauteur))
        elif img02m.get()==1:
            miroir(imggris)
            new_im.paste(imgmiroir,(0*largeur,2*hauteur))
        else:
            new_im.paste(imggris,(0*largeur,2*hauteur))
    if img02.get()=="Noir et blanc":
        if img02r.get()==1:
            retourner(imgnoiretblanc)
            new_im.paste(imgretourner,(0*largeur,2*hauteur))
        elif img02m.get()==1:
            miroir(imgnoiretblanc)
            new_im.paste(imgmiroir,(0*largeur,2*hauteur))
        else:
            new_im.paste(imgnoiretblanc,(0*largeur,2*hauteur))
    if img02.get()=="Negatif":
        if img02r.get()==1:
            retourner(imgnegatif)
            new_im.paste(imgretourner,(0*largeur,2*hauteur))
        elif img02m.get()==1:
            miroir(imgnegatif)
            new_im.paste(imgmiroir,(0*largeur,2*hauteur))
        else:
            new_im.paste(imgnegatif,(0*largeur,2*hauteur))
    if img02.get()=="Mofication couleurs 1":
        if img02r.get()==1:
            retourner(imgchangercouleur1)
            new_im.paste(imgretourner,(0*largeur,2*hauteur))
        elif img02m.get()==1:
            miroir(imgchangercouleur1)
            new_im.paste(imgmiroir,(0*largeur,2*hauteur))
        else:
            new_im.paste(imgchangercouleur1,(0*largeur,2*hauteur))
    if img02.get()=="Mofication couleurs 2":
        if img02r.get()==1:
            retourner(imgchangercouleur2)
            new_im.paste(imgretourner,(0*largeur,2*hauteur))
        elif img02m.get()==1:
            miroir(imgchangercouleur2)
            new_im.paste(imgmiroir,(0*largeur,2*hauteur))
        else:
            new_im.paste(imgchangercouleur2,(0*largeur,2*hauteur))

    #img10
    if img10.get()=="Image de base":
        if img10r.get()==1:
            retourner(img)
            new_im.paste(imgretourner,(1*largeur,0*hauteur))
        elif img10m.get()==1:
            miroir(img)
            new_im.paste(imgmiroir,(1*largeur,0*hauteur))
        else:
            new_im.paste(img,(1*largeur,0*hauteur))
    if img10.get()=="Bruit":
        if img10r.get()==1:
            retourner(imgbruit)
            new_im.paste(imgretourner,(1*largeur,0*hauteur))
        elif img10m.get()==1:
            miroir(imgbruit)
            new_im.paste(imgmiroir,(1*largeur,0*hauteur))
        else:
            new_im.paste(imgbruit,(1*largeur,0*hauteur))
    if img10.get()=="Niveau de gris":
        if img10r.get()==1:
            retourner(imggris)
            new_im.paste(imgretourner,(1*largeur,0*hauteur))
        elif img10m.get()==1:
            miroir(imggris)
            new_im.paste(imgmiroir,(1*largeur,0*hauteur))
        else:
            new_im.paste(imggris,(1*largeur,0*hauteur))
    if img10.get()=="Noir et blanc":
        if img10r.get()==1:
            retourner(imgnoiretblanc)
            new_im.paste(imgretourner,(1*largeur,0*hauteur))
        elif img10m.get()==1:
            miroir(imgnoiretblanc)
            new_im.paste(imgmiroir,(1*largeur,0*hauteur))
        else:
            new_im.paste(imgnoiretblanc,(1*largeur,0*hauteur))
    if img10.get()=="Negatif":
        if img10r.get()==1:
            retourner(imgnegatif)
            new_im.paste(imgretourner,(1*largeur,0*hauteur))
        elif img10m.get()==1:
            miroir(imgnegatif)
            new_im.paste(imgmiroir,(1*largeur,0*hauteur))
        else:
            new_im.paste(imgnegatif,(1*largeur,0*hauteur))
    if img10.get()=="Mofication couleurs 1":
        if img10r.get()==1:
            retourner(imgchangercouleur1)
            new_im.paste(imgretourner,(1*largeur,0*hauteur))
        elif img10m.get()==1:
            miroir(imgchangercouleur1)
            new_im.paste(imgmiroir,(1*largeur,0*hauteur))
        else:
            new_im.paste(imgchangercouleur1,(1*largeur,0*hauteur))
    if img10.get()=="Mofication couleurs 2":
        if img10r.get()==1:
            retourner(imgchangercouleur2)
            new_im.paste(imgretourner,(1*largeur,0*hauteur))
        elif img10m.get()==1:
            miroir(imgchangercouleur2)
            new_im.paste(imgmiroir,(1*largeur,0*hauteur))
        else:
            new_im.paste(imgchangercouleur2,(1*largeur,0*hauteur))

    #img11
    if img11.get()=="Image de base":
        if img11r.get()==1:
            retourner(img)
            new_im.paste(imgretourner,(1*largeur,1*hauteur))
        elif img11m.get()==1:
            miroir(img)
            new_im.paste(imgmiroir,(1*largeur,1*hauteur))
        else:
            new_im.paste(img,(1*largeur,1*hauteur))
    if img11.get()=="Bruit":
        if img11r.get()==1:
            retourner(imgbruit)
            new_im.paste(imgretourner,(1*largeur,1*hauteur))
        elif img11m.get()==1:
            miroir(imgbruit)
            new_im.paste(imgmiroir,(1*largeur,1*hauteur))
        else:
            new_im.paste(imgbruit,(1*largeur,1*hauteur))
    if img11.get()=="Niveau de gris":
        if img11r.get()==1:
            retourner(imggris)
            new_im.paste(imgretourner,(1*largeur,1*hauteur))
        elif img11m.get()==1:
            miroir(imggris)
            new_im.paste(imgmiroir,(1*largeur,1*hauteur))
        else:
            new_im.paste(imggris,(1*largeur,1*hauteur))
    if img11.get()=="Noir et blanc":
        if img11r.get()==1:
            retourner(imgnoiretblanc)
            new_im.paste(imgretourner,(1*largeur,1*hauteur))
        elif img11m.get()==1:
            miroir(imgnoiretblanc)
            new_im.paste(imgmiroir,(1*largeur,1*hauteur))
        else:
            new_im.paste(imgnoiretblanc,(1*largeur,1*hauteur))
    if img11.get()=="Negatif":
        if img11r.get()==1:
            retourner(imgnegatif)
            new_im.paste(imgretourner,(1*largeur,1*hauteur))
        elif img11m.get()==1:
            miroir(imgnegatif)
            new_im.paste(imgmiroir,(1*largeur,1*hauteur))
        else:
            new_im.paste(imgnegatif,(1*largeur,1*hauteur))
    if img11.get()=="Mofication couleurs 1":
        if img11r.get()==1:
            retourner(imgchangercouleur1)
            new_im.paste(imgretourner,(1*largeur,1*hauteur))
        elif img11m.get()==1:
            miroir(imgchangercouleur1)
            new_im.paste(imgmiroir,(1*largeur,1*hauteur))
        else:
            new_im.paste(imgchangercouleur1,(1*largeur,1*hauteur))
    if img11.get()=="Mofication couleurs 2":
        if img11r.get()==1:
            retourner(imgchangercouleur2)
            new_im.paste(imgretourner,(1*largeur,1*hauteur))
        elif img11m.get()==1:
            miroir(imgchangercouleur2)
            new_im.paste(imgmiroir,(1*largeur,1*hauteur))
        else:
            new_im.paste(imgchangercouleur2,(1*largeur,1*hauteur))

    #img12
    if img12.get()=="Image de base":
        if img12r.get()==1:
            retourner(img)
            new_im.paste(imgretourner,(1*largeur,2*hauteur))
        elif img12m.get()==1:
            miroir(img)
            new_im.paste(imgmiroir,(1*largeur,2*hauteur))
        else:
            new_im.paste(img,(1*largeur,2*hauteur))
    if img12.get()=="Bruit":
        if img12r.get()==1:
            retourner(imgbruit)
            new_im.paste(imgretourner,(1*largeur,2*hauteur))
        elif img12m.get()==1:
            miroir(imgbruit)
            new_im.paste(imgmiroir,(1*largeur,2*hauteur))
        else:
            new_im.paste(imgbruit,(1*largeur,2*hauteur))
    if img12.get()=="Niveau de gris":
        if img12r.get()==1:
            retourner(imggris)
            new_im.paste(imgretourner,(1*largeur,2*hauteur))
        elif img12m.get()==1:
            miroir(imggris)
            new_im.paste(imgmiroir,(1*largeur,2*hauteur))
        else:
            new_im.paste(imggris,(1*largeur,2*hauteur))
    if img12.get()=="Noir et blanc":
        if img12r.get()==1:
            retourner(imgnoiretblanc)
            new_im.paste(imgretourner,(1*largeur,2*hauteur))
        elif img12m.get()==1:
            miroir(imgnoiretblanc)
            new_im.paste(imgmiroir,(1*largeur,2*hauteur))
        else:
            new_im.paste(imgnoiretblanc,(1*largeur,2*hauteur))
    if img12.get()=="Negatif":
        if img12r.get()==1:
            retourner(imgnegatif)
            new_im.paste(imgretourner,(1*largeur,2*hauteur))
        elif img12m.get()==1:
            miroir(imgnegatif)
            new_im.paste(imgmiroir,(1*largeur,2*hauteur))
        else:
            new_im.paste(imgnegatif,(1*largeur,2*hauteur))
    if img12.get()=="Mofication couleurs 1":
        if img12r.get()==1:
            retourner(imgchangercouleur1)
            new_im.paste(imgretourner,(1*largeur,2*hauteur))
        elif img12m.get()==1:
            miroir(imgchangercouleur1)
            new_im.paste(imgmiroir,(1*largeur,2*hauteur))
        else:
            new_im.paste(imgchangercouleur1,(1*largeur,2*hauteur))
    if img12.get()=="Mofication couleurs 2":
        if img12r.get()==1:
            retourner(imgchangercouleur2)
            new_im.paste(imgretourner,(1*largeur,2*hauteur))
        elif img12m.get()==1:
            miroir(imgchangercouleur2)
            new_im.paste(imgmiroir,(1*largeur,2*hauteur))
        else:
            new_im.paste(imgchangercouleur2,(1*largeur,2*hauteur))

    #img20
    if img20.get()=="Image de base":
        if img20r.get()==1:
            retourner(img)
            new_im.paste(imgretourner,(2*largeur,0*hauteur))
        elif img20m.get()==1:
            miroir(img)
            new_im.paste(imgmiroir,(2*largeur,0*hauteur))
        else:
            new_im.paste(img,(2*largeur,0*hauteur))
    if img20.get()=="Bruit":
        if img20r.get()==1:
            retourner(imgbruit)
            new_im.paste(imgretourner,(2*largeur,0*hauteur))
        elif img20m.get()==1:
            miroir(imgbruit)
            new_im.paste(imgmiroir,(2*largeur,0*hauteur))
        else:
            new_im.paste(imgbruit,(2*largeur,0*hauteur))
    if img20.get()=="Niveau de gris":
        if img20r.get()==1:
            retourner(imggris)
            new_im.paste(imgretourner,(2*largeur,0*hauteur))
        elif img20m.get()==1:
            miroir(imggris)
            new_im.paste(imgmiroir,(2*largeur,0*hauteur))
        else:
            new_im.paste(imggris,(2*largeur,0*hauteur))
    if img20.get()=="Noir et blanc":
        if img20r.get()==1:
            retourner(imgnoiretblanc)
            new_im.paste(imgretourner,(2*largeur,0*hauteur))
        elif img20m.get()==1:
            miroir(imgnoiretblanc)
            new_im.paste(imgmiroir,(2*largeur,0*hauteur))
        else:
            new_im.paste(imgnoiretblanc,(2*largeur,0*hauteur))
    if img20.get()=="Negatif":
        if img20r.get()==1:
            retourner(imgnegatif)
            new_im.paste(imgretourner,(2*largeur,0*hauteur))
        elif img20m.get()==1:
            miroir(imgnegatif)
            new_im.paste(imgmiroir,(2*largeur,0*hauteur))
        else:
            new_im.paste(imgnegatif,(2*largeur,0*hauteur))
    if img20.get()=="Mofication couleurs 1":
        if img20r.get()==1:
            retourner(imgchangercouleur1)
            new_im.paste(imgretourner,(2*largeur,0*hauteur))
        elif img20m.get()==1:
            miroir(imgchangercouleur1)
            new_im.paste(imgmiroir,(2*largeur,0*hauteur))
        else:
            new_im.paste(imgchangercouleur1,(2*largeur,0*hauteur))
    if img20.get()=="Mofication couleurs 2":
        if img20r.get()==1:
            retourner(imgchangercouleur2)
            new_im.paste(imgretourner,(2*largeur,0*hauteur))
        elif img20m.get()==1:
            miroir(imgchangercouleur2)
            new_im.paste(imgmiroir,(2*largeur,0*hauteur))
        else:
            new_im.paste(imgchangercouleur2,(2*largeur,0*hauteur))

    #img21
    if img21.get()=="Image de base":
        if img21r.get()==1:
            retourner(img)
            new_im.paste(imgretourner,(2*largeur,1*hauteur))
        elif img21m.get()==1:
            miroir(img)
            new_im.paste(imgmiroir,(2*largeur,1*hauteur))
        else:
            new_im.paste(img,(2*largeur,1*hauteur))
    if img21.get()=="Bruit":
        if img21r.get()==1:
            retourner(imgbruit)
            new_im.paste(imgretourner,(2*largeur,1*hauteur))
        elif img21m.get()==1:
            miroir(imgbruit)
            new_im.paste(imgmiroir,(2*largeur,1*hauteur))
        else:
            new_im.paste(imgbruit,(2*largeur,1*hauteur))
    if img21.get()=="Niveau de gris":
        if img21r.get()==1:
            retourner(imggris)
            new_im.paste(imgretourner,(2*largeur,1*hauteur))
        elif img21m.get()==1:
            miroir(imggris)
            new_im.paste(imgmiroir,(2*largeur,1*hauteur))
        else:
            new_im.paste(imggris,(2*largeur,1*hauteur))
    if img21.get()=="Noir et blanc":
        if img21r.get()==1:
            retourner(imgnoiretblanc)
            new_im.paste(imgretourner,(2*largeur,1*hauteur))
        elif img21m.get()==1:
            miroir(imgnoiretblanc)
            new_im.paste(imgmiroir,(2*largeur,1*hauteur))
        else:
            new_im.paste(imgnoiretblanc,(2*largeur,1*hauteur))
    if img21.get()=="Negatif":
        if img21r.get()==1:
            retourner(imgnegatif)
            new_im.paste(imgretourner,(2*largeur,1*hauteur))
        elif img21m.get()==1:
            miroir(imgnegatif)
            new_im.paste(imgmiroir,(2*largeur,1*hauteur))
        else:
            new_im.paste(imgnegatif,(2*largeur,1*hauteur))
    if img21.get()=="Mofication couleurs 1":
        if img21r.get()==1:
            retourner(imgchangercouleur1)
            new_im.paste(imgretourner,(2*largeur,1*hauteur))
        elif img21m.get()==1:
            miroir(imgchangercouleur1)
            new_im.paste(imgmiroir,(2*largeur,1*hauteur))
        else:
            new_im.paste(imgchangercouleur1,(2*largeur,1*hauteur))
    if img21.get()=="Mofication couleurs 2":
        if img21r.get()==1:
            retourner(imgchangercouleur2)
            new_im.paste(imgretourner,(2*largeur,1*hauteur))
        elif img21m.get()==1:
            miroir(imgchangercouleur2)
            new_im.paste(imgmiroir,(2*largeur,1*hauteur))
        else:
            new_im.paste(imgchangercouleur2,(2*largeur,1*hauteur))

    #img22
    if img22.get()=="Image de base":
        if img22r.get()==1:
            retourner(img)
            new_im.paste(imgretourner,(2*largeur,2*hauteur))
        elif img22m.get()==1:
            miroir(img)
            new_im.paste(imgmiroir,(2*largeur,2*hauteur))
        else:
            new_im.paste(img,(2*largeur,2*hauteur))
    if img22.get()=="Bruit":
        if img22r.get()==1:
            retourner(imgbruit)
            new_im.paste(imgretourner,(2*largeur,2*hauteur))
        elif img22m.get()==1:
            miroir(imgbruit)
            new_im.paste(imgmiroir,(2*largeur,2*hauteur))
        else:
            new_im.paste(imgbruit,(2*largeur,2*hauteur))
    if img22.get()=="Niveau de gris":
        if img22r.get()==1:
            retourner(imggris)
            new_im.paste(imgretourner,(2*largeur,2*hauteur))
        elif img22m.get()==1:
            miroir(imggris)
            new_im.paste(imgmiroir,(2*largeur,2*hauteur))
        else:
            new_im.paste(imggris,(2*largeur,2*hauteur))
    if img22.get()=="Noir et blanc":
        if img22r.get()==1:
            retourner(imgnoiretblanc)
            new_im.paste(imgretourner,(2*largeur,2*hauteur))
        elif img22m.get()==1:
            miroir(imgnoiretblanc)
            new_im.paste(imgmiroir,(2*largeur,2*hauteur))
        else:
            new_im.paste(imgnoiretblanc,(2*largeur,2*hauteur))
    if img22.get()=="Negatif":
        if img22r.get()==1:
            retourner(imgnegatif)
            new_im.paste(imgretourner,(2*largeur,2*hauteur))
        elif img22m.get()==1:
            miroir(imgnegatif)
            new_im.paste(imgmiroir,(2*largeur,2*hauteur))
        else:
            new_im.paste(imgnegatif,(2*largeur,2*hauteur))
    if img22.get()=="Mofication couleurs 1":
        if img22r.get()==1:
            retourner(imgchangercouleur1)
            new_im.paste(imgretourner,(2*largeur,2*hauteur))
        elif img22m.get()==1:
            miroir(imgchangercouleur1)
            new_im.paste(imgmiroir,(2*largeur,2*hauteur))
        else:
            new_im.paste(imgchangercouleur1,(2*largeur,2*hauteur))
    if img22.get()=="Mofication couleurs 2":
        if img22r.get()==1:
            retourner(imgchangercouleur2)
            new_im.paste(imgretourner,(2*largeur,2*hauteur))
        elif img22m.get()==1:
            miroir(imgchangercouleur2)
            new_im.paste(imgmiroir,(2*largeur,2*hauteur))
        else:
            new_im.paste(imgchangercouleur2,(2*largeur,2*hauteur))

    return new_im

def afficherapercu():
    """ouvre un apercu temporaire de l'image finale
    ENTREE: rien
    SORTIE: une fenetre avec l'image finale"""
    imagefinale()
    new_im.show()

def enregistrer():
    """ouvre un popup pour demander le chemin ou enregistrer l'image finale puis l'enregistre a cet endroit
    ENTREE: rien
    SORTIE: l'image finale au chemin indique par l'utilisateur"""
    global savechemin
    imagefinale()
    savechemin=filedialog.asksaveasfilename(parent=fenetre,title="Enregistrer un fichier",filetypes=[("Image","*.png")],defaultextension=".png")
    new_im.save(savechemin)

def retourner(img_a_retourner):
    """prend l'image et la renvoie retourner
    ENTREE: une image
    SORTIE: l'image retourne"""
    global imgretourner
    largeur,hauteur=img_a_retourner.size
    imgretourner=Image.new("RGB",(largeur, hauteur))
    for y in range(hauteur):
        for x in range(largeur):
            r,v,b=img_a_retourner.getpixel((x,y))
            imgretourner.putpixel((largeur-1-x,hauteur-1-y),(r,v,b))
    return imgretourner

def miroir(img_a_miroiter):
    """prend l'image et la renvoie en miroir
    ENTREE: une image
    SORTIE: l'image en miroir"""
    global imgmiroir
    largeur,hauteur=img_a_miroiter.size
    imgmiroir=Image.new("RGB",img_a_miroiter.size)
    for x in range(largeur):
        x_miroir=largeur-x-1
        for y in range(hauteur):
            pixel=img_a_miroiter.getpixel((x,y))
            imgmiroir.putpixel((x_miroir,y),pixel)
    return imgmiroir

def bruit():
    """prend l'image et la renvoie avec du bruit
    ENTREE: une image
    SORTIE: l'image avec du bruit aleatoire"""
    global imgbruit
    imgbruit=Image.open(nomfichier)
    largeur,hauteur=imgbruit.size
    for y in range(hauteur):
        for x in range(largeur):
            u=randrange(-100,100)
            z=randrange(-100,100)
            l=randrange(-100,100)
            r,v,b=imgbruit.getpixel((x,y))
            imgbruit.putpixel((x,y),(r+u,v+z,b+l))
    return imgbruit

def niveaudegris():
    """prend l'image et la renvoie en niveau de gris
    ENTREE: une image
    SORTIE: l'image en niveau de gris"""
    global imggris
    imggris=Image.open(nomfichier)
    largeur,hauteur=imggris.size
    for y in range(hauteur):
        for x in range(largeur):
            r,v,b=imggris.getpixel((x,y))
            imggris.putpixel((x,y),(int(r+v+b/3),int(r+v+b/3),int(r+v+b/3)))
    return imggris

def noiretblanc():
    """prend l'image et la renvoie en noir et blanc
    ENTREE: une image
    SORTIE: l'image en noir et blanc"""
    global imgnoiretblanc
    imgnoiretblanc=Image.open(nomfichier)
    largeur,hauteur=imgnoiretblanc.size
    for y in range(hauteur):
        for x in range(largeur):
            r,v,b=imgnoiretblanc.getpixel((x,y))
            m=int(r+v+b/3)
            if m>180:
                m=(255,255,255)
            else:
                m=(0,0,0)
            imgnoiretblanc.putpixel((x,y),(m))
    return imgnoiretblanc

def negatif():
    """prend l'image et la renvoie en negatif
    ENTREE: une image
    SORTIE: l'image en negatif"""
    global imgnegatif
    imgnegatif=Image.open(nomfichier)
    largeur,hauteur=imgnegatif.size
    for y in range(hauteur):
        for x in range(largeur):
            r,v,b=imgnegatif.getpixel((x,y))
            imgnegatif.putpixel((x,y),(255-r,255-v,255-b))
    return imgnegatif

def changercouleur1():
    """prend l'image et la renvoie avec des couleurs modifiees (1)
    ENTREE: une image
    SORTIE: l'image avec des couleurs modifiees (1)"""
    global imgchangercouleur1
    imgchangercouleur1=Image.open(nomfichier)
    largeur,hauteur=imgchangercouleur1.size
    for y in range(hauteur):
        for x in range(largeur):
            r,v,b=imgchangercouleur1.getpixel((x,y))
            imgchangercouleur1.putpixel((x,y),(v,b,r))
    return imgchangercouleur1

def changercouleur2():
    """prend l'image et la renvoie avec des couleurs modifiees (2)
    ENTREE: une image
    SORTIE: l'image avec des couleurs modifiees (2)"""
    global imgchangercouleur2
    imgchangercouleur2=Image.open(nomfichier)
    largeur,hauteur=imgchangercouleur2.size
    for y in range(hauteur):
        for x in range(largeur):
            r,v,b=imgchangercouleur2.getpixel((x,y))
            imgchangercouleur2.putpixel((x,y),(b,r,v))
    return imgchangercouleur2

def fram1versfram2() :
    """permet d'appeler la fonction pour fermer la frame1ere et la fontion pour ouvrir/creer la f2 en verifiant si on a selectionner une image et si on a un nb de colonnes et de lignes valides
    ENTREE: rien
    SORTIE: appele la fonction pour fermer la frame1ere et la fonction pour ouvrir/creer la f2"""
    if 'nomfichier' in globals():
        if nomfichier=="":
            #affiche une popup qui nous dis qu'il faut selectionner une image
            popuppasdenom=CTkToplevel()
            popuppasdenom.title("Erreur")
            popuppasdenom.geometry("300x50")
            popuppasdenom.minsize(250,50)
            popuppasdenom.iconphoto(False,iconeerreur)
            textepasdenom=Label(popuppasdenom,text="Veuillez selectionner une image !",text_font=("Roboto Medium", 13))
            textepasdenom.pack(expand=YES)
        else:
            if 'nb_lignes' in globals() or 'nb_colonnes' in globals():
                if nb_lignes not in (1,2,3) or nb_colonnes not in (1,2,3):
                    #affiche une popup qui nous dis qu'il faut renseigner des nombres de lignes et de colonnes compris entre 1 et 3
                    popupmauvaisnbs=CTkToplevel()
                    popupmauvaisnbs.title("Erreur")
                    popupmauvaisnbs.geometry("620x50")
                    popupmauvaisnbs.minsize(620,50)
                    popupmauvaisnbs.iconphoto(False,iconeerreur)
                    textemauvaisnbs=CTkLabel(popupmauvaisnbs,text="Veuillez renseigner des nombres de lignes et de colonnes compris entre 1 et 3 !",text_font=("Roboto Medium", 13))
                    textemauvaisnbs.pack(expand=YES)
                else:
                    #tout est correct, on passe a la suite
                    close_frame1ere()
                    create_f2()
            else:
                #affiche une popup qui nous dis qu'il faut cliquer sur le bouton valider des lignes et des colonnes
                popupvalider=CTkToplevel()
                popupvalider.title("Erreur")
                popupvalider.geometry("700x50")
                popupvalider.minsize(700,50)
                popupvalider.iconphoto(False,iconeerreur)
                textevalider=CTkLabel(popupvalider,text="Veuillez cliquer sur les boutons 'Valider' pour valider le nombres de lignes et de colonnes !",text_font=("Roboto Medium", 13))
                textevalider.pack(expand=YES)
    else:
        #affiche une popup qui nous dis qu'il faut selectionner une image
        popuppasdenom2=CTkToplevel()
        popuppasdenom2.title("Erreur")
        popuppasdenom2.geometry("300x50")
        popuppasdenom2.minsize(250,50)
        popuppasdenom2.iconphoto(False,iconeerreur)
        textepasdenom2=CTkLabel(popuppasdenom2,text="Veuillez selectionner une image !",text_font=("Roboto Medium", 13))
        textepasdenom2.pack(expand=YES)

def fonc_nb_colonnes():
    """utilisee pour prendre la valeur du champ colonnes et le stocker dans la variable nb_colonnes
    ENTREE: rien
    SORTIE: varible nb_colonnes"""
    global nb_colonnes
    nb_colonnes=IntVar()
    nb_colonnes=int(user_nb_colonnes.get())

def fonc_nb_lignes():
    """utilisee pour prendre la valeur du champ lignes et le stocker dans la variable nb_lignes
    ENTREE: rien
    SORTIE: varible nb_lignes"""
    global nb_lignes
    nb_lignes=IntVar()
    nb_lignes=int(user_nb_lignes.get())

def fram2versfram1() :
    """permet d'appeler la fonction pour fermer la f2 et la fontion pour ouvrir/creer la frame1ere
    ENTREE: rien
    SORTIE: appele la fonction pour fermer la f2 et la fonction pour ouvrir/creer la frame1ere"""
    close_f2()
    create_frame1ere()

def close_frame1ere():
    """permet de fermer la frame1ere
    ENTREE: rien
    SORTIE: ferme la frame1ere"""
    global frame1ere
    frame1ere.destroy()

def close_f2():
    """permet de fermer la f2
    ENTREE: rien
    SORTIE: ferme f2"""
    global f2
    f2.destroy()

def create_frame1ere():
    """permet de ouvrir/creer la frame1ere
    ENTREE: rien
    SORTIE: ouvre/cree la frame1ere"""
    global frame1ere, textebienvenue, texteexplicationapp, boutonchoosefile, boutonbruit, colonnes, lignes, user_nb_colonnes, boutonvalidercolonnes, user_nb_lignes, boutonvaliderlignes, texteexplicationlignes, texteexplicationcolonnes

    #parametres frame
    frame1ere=CTkFrame(fenetre)

    #texte explicatif
    texteexplicationapp=CTkLabel(frame1ere,text="Bienvenue ! Dans ce programme, vous pouvez a partir d'une image en creer une multitude avec differents effets, puis les rassembler dans une seule image.",text_font=("Roboto Medium",15),text_color='white',wraplengt=570)
    texteexplicationapp.pack(pady=5)

    #bouton choisir fichier
    boutonchoosefile=CTkButton(frame1ere,text="Choisir une image",fg_color='#ca4d31',hover_color='#4A1C12',text_font=("Roboto Medium", 15),text_color='white',width=300,height=50,command=ouvrir_fichier)
    boutonchoosefile.pack(pady=15)

    #champs pour saisir nb colonnes + textes explicatifs + bouton valider
    texteexplicationcolonnes=CTkLabel(frame1ere,text="Rentrez ici le nombre de colonnes souhaité pour l'image finale",text_font=("Roboto Medium", 15),text_color='white',wraplengt=550)
    texteexplicationcolonnes.pack(pady=10)
    user_nb_colonnes=IntVar()
    colonnes=CTkEntry(frame1ere,text_font=("Roboto Medium",20),placeholder_text_color='white',textvariable=user_nb_colonnes,width=300)
    colonnes.pack()
    boutonvalidercolonnes=CTkButton(frame1ere,text="Valider",text_font=("Roboto Medium", 10),text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',command=fonc_nb_colonnes,width=80,height=30)
    boutonvalidercolonnes.pack(pady=10)

    #champs pour saisir nb lignes + textes explicatifs + bouton valider
    texteexplicationlignes=CTkLabel(frame1ere,text="Rentrez ici le nombre de lignes souhaité pour l'image finale",text_font=("Roboto Medium", 15),text_color='white',wraplengt=550)
    texteexplicationlignes.pack(pady=10)
    user_nb_lignes=IntVar()
    lignes=CTkEntry(frame1ere,text_font=("Roboto Medium",20),placeholder_text_color='white',textvariable=user_nb_lignes,width=300)
    lignes.pack()
    boutonvaliderlignes=CTkButton(frame1ere,text="Valider",text_font=("Roboto Medium", 10),text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',command=fonc_nb_lignes,width=80,height=30)
    boutonvaliderlignes.pack(pady=10)

    #bouton etape suivante
    boutonetapesuivante=CTkButton(frame1ere, text='Etape suivante',text_font=("Roboto Medium", 15),text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',command=fram1versfram2,width=150,height=40)
    boutonetapesuivante.pack(pady=20)

    #creation frame
    frame1ere.pack(expand=YES)

def create_f2():
    """permet de ouvrir/creer la 2
    ENTREE: rien
    SORTIE: ouvre/cree la 2"""
    global f2, listeoptions, menuderoulant, img00, img01, img02, img10, img11, img12, img20, img21, img22, img00r, img01r, img02r, img10r, img11r, img12r, img20r, img21r, img22r, img00m, img01m, img02m, img10m, img11m, img12m, img20m, img21m, img22m

    #parametres frame
    f2=CTkFrame(fenetre)

    #texte explication2 (titre)
    texteexplication2=CTkLabel(f2,text="Choississez les effets que vous souhaitez pour chaque image",text_font=("Roboto Medium",20),text_color='white')
    texteexplication2.grid(row=0,pady=20,columnspan=3)

    #menus deroulants pour choisir les effets
    listeoptions=["Image de base","Bruit","Niveau de gris","Noir et blanc","Negatif","Mofication couleurs 1","Mofication couleurs 2"]
    img00=StringVar(f2)
    img00r=IntVar(f2)
    img00m=IntVar(f2)
    img01=StringVar(f2)
    img01r=IntVar(f2)
    img01m=IntVar(f2)
    img02=StringVar(f2)
    img02r=IntVar(f2)
    img02m=IntVar(f2)
    img10=StringVar(f2)
    img10r=IntVar(f2)
    img10m=IntVar(f2)
    img11=StringVar(f2)
    img11r=IntVar(f2)
    img11m=IntVar(f2)
    img12=StringVar(f2)
    img12r=IntVar(f2)
    img12m=IntVar(f2)
    img20=StringVar(f2)
    img20r=IntVar(f2)
    img20m=IntVar(f2)
    img21=StringVar(f2)
    img21r=IntVar(f2)
    img21m=IntVar(f2)
    img22=StringVar(f2)
    img22r=IntVar(f2)
    img22m=IntVar(f2)
    if nb_colonnes==1 and nb_lignes==1:
        #img00
        menuderoulant=CTkOptionMenu(f2,variable=img00,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=1,pady=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img00r, onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img00m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=3,pady=5)
    if nb_colonnes==1 and nb_lignes==2:
        #img00
        menuderoulant=CTkOptionMenu(f2,variable=img00,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=1,pady=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img00r, onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img00m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=3,pady=5)
        
        #img01
        menuderoulant=CTkOptionMenu(f2,variable=img01,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=4,pady=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img01r, onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img01m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=6,pady=5)
    if nb_colonnes==1 and nb_lignes==3:
        #img00
        menuderoulant=CTkOptionMenu(f2,variable=img00,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=1,pady=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img00r, onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img00m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=3,pady=5)
        
        #img01
        menuderoulant=CTkOptionMenu(f2,variable=img01,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=4,pady=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img01r, onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img01m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=6,pady=5)
        
        #img02
        menuderoulant=CTkOptionMenu(f2,variable=img02,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=7,pady=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img02r, onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=8,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img02m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=9,pady=5)
    if nb_colonnes==2 and nb_lignes==1:
        #img00
        menuderoulant=CTkOptionMenu(f2,variable=img00,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img00r, onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img00m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=3,pady=5)

        #img10
        menuderoulant=CTkOptionMenu(f2,variable=img10,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img10r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img10m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=3,pady=5)
    if nb_colonnes==2 and nb_lignes==2:
        #img00
        menuderoulant=CTkOptionMenu(f2,variable=img00,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img00r, onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img00m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=3,pady=5)
        
        #img01
        menuderoulant=CTkOptionMenu(f2,variable=img01,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img01r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img01m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=6,pady=5)

        #img10
        menuderoulant=CTkOptionMenu(f2,variable=img10,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img10r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img10m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=3,pady=5)

        #img11
        menuderoulant=CTkOptionMenu(f2,variable=img11,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img11r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img11m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=6,pady=5)
    if nb_colonnes==2 and nb_lignes==3:
        #img00
        menuderoulant=CTkOptionMenu(f2,variable=img00,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img00r, onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img00m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=3,pady=5)
        
        #img01
        menuderoulant=CTkOptionMenu(f2,variable=img01,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img01r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img01m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=6,pady=5)

        #img02
        menuderoulant=CTkOptionMenu(f2,variable=img02,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=7,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img02r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=8,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img02m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=9,pady=5)

        #img10
        menuderoulant=CTkOptionMenu(f2,variable=img10,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img10r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img10m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=3,pady=5)

        #img11
        menuderoulant=CTkOptionMenu(f2,variable=img11,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img11r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img11m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=6,pady=5)

        #img12
        menuderoulant=CTkOptionMenu(f2,variable=img12,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=7,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img12r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=8,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img12m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=9,pady=5)
    if nb_colonnes==3 and nb_lignes==1:
        #img00
        menuderoulant=CTkOptionMenu(f2,variable=img00,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img00r, onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img00m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=3,pady=5)

        #img10
        menuderoulant=CTkOptionMenu(f2,variable=img10,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img10r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img10m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=3,pady=5)

        #img20
        menuderoulant=CTkOptionMenu(f2,variable=img20,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=2,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img20r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=2,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img20m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=2,row=3,pady=5)
    if nb_colonnes==3 and nb_lignes==2:
        #img00
        menuderoulant=CTkOptionMenu(f2,variable=img00,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img00r, onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img00m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=3,pady=5)

        #img01
        menuderoulant=CTkOptionMenu(f2,variable=img01,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img01r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img01m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=6,pady=5)

        #img10
        menuderoulant=CTkOptionMenu(f2,variable=img10,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img10r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img10m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=3,pady=5)

        #img11
        menuderoulant=CTkOptionMenu(f2,variable=img11,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img11r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img11m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=6,pady=5)

        #img20
        menuderoulant=CTkOptionMenu(f2,variable=img20,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=2,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img20r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=2,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img20m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=2,row=3,pady=5)
        
        #img21
        menuderoulant=CTkOptionMenu(f2,variable=img21,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=2,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img21r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=2,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img21m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=2,row=6,pady=5)
    if nb_colonnes==3 and nb_lignes==3:
        #img00
        menuderoulant=CTkOptionMenu(f2,variable=img00,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2, text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),variable=img00r, onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img00m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=3,pady=5)

        #img01
        menuderoulant=CTkOptionMenu(f2,variable=img01,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img01r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img01m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=6,pady=5)

        #img02
        menuderoulant=CTkOptionMenu(f2,variable=img02,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=0,row=7,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img02r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=0,row=8,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img02m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=0,row=9,pady=5)

        #img10
        menuderoulant=CTkOptionMenu(f2,variable=img10,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img10r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img10m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=3,pady=5)

        #img11
        menuderoulant=CTkOptionMenu(f2,variable=img11,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img11r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img11m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=6,pady=5)

        #img12
        menuderoulant=CTkOptionMenu(f2,variable=img12,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=1,row=7,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img12r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=1,row=8,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img12m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=1,row=9,pady=5)

        #img20
        menuderoulant=CTkOptionMenu(f2,variable=img20,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=2,row=1,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img20r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=2,row=2,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img20m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=2,row=3,pady=5)
        
        #img21
        menuderoulant=CTkOptionMenu(f2,variable=img21,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=2,row=4,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img21r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=2,row=5,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img21m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=2,row=6,pady=5)
        
        #img22
        menuderoulant=CTkOptionMenu(f2,variable=img22,hover=True,values=listeoptions,width=250,height=35,button_color='#ca4d31',button_hover_color='#4A1C12',dropdown_hover_color='#4A1C12',fg_color='#4A1C12',text_color='white',text_font=('Roboto Medium',15),dropdown_text_font=('Roboto Medium',10))
        menuderoulant.set("Image de base")
        menuderoulant.grid(column=2,row=7,pady=5,padx=5)
        checkbox=CTkCheckBox(f2,variable=img22r,text='Retourner ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12), onvalue=1, offvalue=0)
        checkbox.grid(column=2,row=8,pady=5)
        checkbox2=CTkCheckBox(f2,variable=img22m,text='Miroir ?',text_color='white',fg_color='#ca4d31',hover_color='#4A1C12',text_font=('Roboto Medium',12),onvalue=1, offvalue=0)
        checkbox2.grid(column=2,row=9,pady=5)

    #bouton annuler
    boutonannuler=CTkButton(f2, text='Annuler',text_font=("Roboto medium", 15),fg_color='#ca4d31',hover_color='#4A1C12',width=230,height=40,command=fram2versfram1)
    boutonannuler.grid(column=0,row=nb_lignes*3+1,padx=10,pady=20)

    #bouton apercu de l'image
    boutonannuler=CTkButton(f2, text="Apercu de l'image finale",text_font=("Roboto medium", 15),fg_color='#ca4d31',hover_color='#4A1C12',width=230,height=40,command=afficherapercu)
    boutonannuler.grid(column=1,row=nb_lignes*3+1,padx=10,pady=20)

    #bouton enregistrer l'image
    boutonannuler=CTkButton(f2, text="Enregistrer l'image finale",text_font=("Roboto medium", 15),fg_color='#ca4d31',hover_color='#4A1C12',width=230,height=40,command=enregistrer)
    boutonannuler.grid(column=2,row=nb_lignes*3+1,padx=10,pady=20)

    #creation frame
    f2.pack(expand=YES)

#parametres fenetre principale
set_appearance_mode("dark")
set_default_color_theme("blue")
fenetre=CTk()
fenetre.title("Createur d'oeuvres d'art")
fenetre.geometry("800x600")
fenetre.minsize(800,600)
icone_data="""R0lGODlhAAGrAPcAAAAAAGgWBHYZBHEOA3slB10NAIYcDqYcDIcnB5
"""
iconefleur=PhotoImage(data=icone_data)
fenetre.iconphoto(False,iconefleur)
fenetre.configure(fg_color='#2A2D2E')

#stockage icone erreur en base 64
icone_erreur_data="""R0lGODlhAAHhAPcAAAAAAAEBAQgHAgwLAhAPAxQTAxgXAxwaBCEfBC

ltfdVHXXfltVdffSwS0V+HJbZYY4eNNVlcNWo1IAA7"""
iconeerreur=PhotoImage(data=icone_erreur_data)

#mise en place de fenetre et de frame
create_frame1ere()
fenetre.mainloop()